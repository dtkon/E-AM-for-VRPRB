...
class TSPEnv:
    ...
    def step(self, selected):
        ...
        # shape: (batch, pomo)
        before_mask = self.step_state.ninf_mask.clone()
        ...
        finished = (
            (self.step_state.ninf_mask == float('-inf'))
            .all(dim=2, keepdim=True)
            .expand_as(self.step_state.ninf_mask)
        )
        self.step_state.ninf_mask = torch.where(
            finished, before_mask, self.step_state.ninf_mask
        )
    ...
    def _get_travel_distance(self):
        actual_problem_size = self.selected_node_list.size(2)

        gathering_index = self.selected_node_list.unsqueeze(3).expand(
            self.batch_size, -1, actual_problem_size, 2
        )
