...
class TSPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, extra_ninf_mask: Optional[torch.Tensor] = None):
        self.encoded_nodes = self.encoder(reset_state.problems, extra_ninf_mask)
        # shape: (batch, problem, EMBEDDING_DIM)
    ...
    def forward(self, state):
        ...
        if state.current_node is None:
            ...
            # avoid masked nodes
            ninf_mask = state.ninf_mask[:, 0, :] # (batch_size, problem_size)
...
class TSP_Encoder(nn.Module):
    ...
    def forward(self, data, extra_ninf_mask):
