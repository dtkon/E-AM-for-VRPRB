...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state, extra_ninf_mask: Optional[torch.Tensor] = None):
        ...
        self.encoded_nodes = self.encoder(depot_xy, node_xy_demand, extra_ninf_mask)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            # avoid masked nodes
            ninf_mask = state.ninf_mask[:, 0, 1:] # (batch_size, customer_size)
            selected_float = selected + ninf_mask
            select_depot = torch.zeros(size=(batch_size, pomo_size), dtype=torch.long)
            selected = torch.where(selected_float < 0, select_depot, selected)
...
class CVRP_Encoder(nn.Module):
    ...
    def forward(self, depot_xy, node_xy_demand, extra_ninf_mask):
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, input1, extra_ninf_mask):
        ...
        out_concat = multi_head_attention(q, k, v, rank2_ninf_mask=extra_ninf_mask)
