...
from .my_CVRPEnv import my_CVRPEnv as Env
from .CVRPModel import CVRPModel as Model

FILE_DIR_PATH = os.path.dirname(os.path.abspath(__file__))


class CvrpSolver(Solver):
    def __init__(
        self,
        model_load_path: str = 'saved_my_CVRP20_model',
    ...
    def to_device(self, device: Union[str, torch.device]) -> 'CvrpSolver':
    ...
    def solve(
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor]:  # modified from CVRPTester._test_one_batch
        '''
        ...
        return: [(batch_size,), (batch_size, node_indexes)]
        '''
        ...
        customer_numbers = problem_size - 1

        if problem_size != self.problem_size:
            self.env_params = {
                'problem_size': customer_numbers,
                'pomo_size': customer_numbers,
            }
            self.env = Env(**self.env_params)
        ...
        if mask is not None:
            self.env.visited_ninf_flag += extra_ninf_mask_for_decoder
        ...
        route = torch.stack(
            route_list, dim=-1
        )  # (augmentation*batch, pomo, node_indexes)
        ...
        # 追踪到的路线结尾可能都有多个连续0的原因是这是从pomo中取的最短路，
        # 如果没有从pomo中取的话就不会有多个连续0了。
